    ...
    return (
        <div className = 'px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden'>
            ...
            <div className = 'relative mt-6'>
                ...
                <ReactPlayer className = 'mx-auto max-w-full' width = '960px' height = '540px' url = { dummyTrailers[currentTrailer].videoUrl } controls = { false }/>
            </div>
            <div className = 'group grid grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto'>
                {dummyTrailers.map((trailer, index) => (
                    <div onClick = { () => setCurrentTrailer(index) } className = 'relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor-pointer' key = { index }>
                        <img className = 'rounded-lg w-full h-full object-cover brightness-75' src = {trailer.image} alt = ''/>
